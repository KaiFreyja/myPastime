...
class TalkContentController extends Controller
{
    /**
    ...
    public function getAllTalkContent(Request $request)
    {
        try
        {
    ...
    public function getOneTalkContent(Request $request,$id)
    {
		try {
	 ...
	 * @return Response
	 */
	public function putTalkContent(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
	 ...
	 * @return Response
	 */
	public function deleteTalkContent(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new TalkContent ; 
			$pky = $model->primaryKey ;
			$query = TalkContent::where($pky , $id)->delete();
