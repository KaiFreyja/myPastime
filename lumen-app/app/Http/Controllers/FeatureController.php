...
class FeatureController extends Controller
{
    ...
    public function getAllFeature(Request $request)
    {
    	...
    	public function getOneFeature(Request $request,$id){
		try {
	 ...
	 * @return Response
	 */
	public function putFeature(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = Feature::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = Feature::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = Feature::where('code',$request->code)->get();
	...
	public function deleteRole(Request $request,$id){
		...
		try {
			...
			$model = new Feature ; 
			$pky = $model->primaryKey ;
			$query = Feature::where($pky , $id)->delete();
