...
class RarityWeightsController extends Controller
{
    /**
    ...
    public function getAllRarityWeights(Request $request)
    {
        try
        {
    ...
    public function getOneRarityWeights(Request $request,$id)
    {
		try {
    ...
    public function postRarityWeights(Request $request){
	 ...
	 * @return Response
	 */
	public function putRarityWeights(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = RarityWeights::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = RarityWeights::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = RarityWeights::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteRarityWeights(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new RarityWeights ; 
			$pky = $model->primaryKey ;
			$query = RarityWeights::where($pky , $id)->delete();
