...
class GachaponGroupController extends Controller
{
    /**
    ...
    public function getAllGachaponGroup(Request $request)
    {
        try
        {
    ...
    public function getOneGachaponGroup(Request $request,$id)
    {
		try {
    ...
    public function postGachaponGroup(Request $request){
	 ...
	 * @return Response
	 */
	public function putGachaponGroup(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = GachaponGroup::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = GachaponGroup::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = GachaponGroup::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteGachaponGroup(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new GachaponGroup ; 
			$pky = $model->primaryKey ;
			$query = GachaponGroup::where($pky , $id)->delete();
