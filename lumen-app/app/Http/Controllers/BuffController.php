...
class BuffController extends Controller
{
    ...
    public function getOneBuff(Request $request,$id)
    {
		try {
    ...
    public function postBuff(Request $request){
	 ...
	 * @return Response
	 */
	public function putBuff(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = Buff::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = Buff::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = Skill::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteBuff(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new Buff ; 
			$pky = $model->primaryKey ;
			$query = Buff::where($pky , $id)->delete();
