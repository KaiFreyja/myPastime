...
use App\Models\MapPos;

class MapPosController extends Controller
{
    /**
    ...
    public function getAllMapPos(Request $request)
    {
        try
        {
    ...
    public function getOneMapPos(Request $request,$id)
    {
		try {
    ...
    public function postMapPos(Request $request){
	 ...
	 * @return Response
	 */
	public function putMapPos(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = MapPos::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = MapPos::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = MapPos::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteMapPos(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new MapPos ; 
			$pky = $model->primaryKey ;
			$query = MapPos::where($pky , $id)->delete();
