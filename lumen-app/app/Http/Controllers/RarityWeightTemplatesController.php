...
class RarityWeightTemplatesController extends Controller
{
    /**
    ...
    public function getAllRarityWeightTemplates(Request $request)
    {
        try
        {
            ...
            $model = new RarityWeightTemplates();
    ...
    public function getOneRarityWeightTemplates(Request $request,$id)
    {
		try {
			...
			$api_name = $this->getApiName($request->path());
            $res = RarityWeightTemplates::find($id);
    ...
    public function postRarityWeightTemplates(Request $request){
		...
		try {
			...
			$query = new RarityWeightTemplates ; 
	 ...
	 * @return Response
	 */
	public function putRarityWeightTemplates(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = RarityWeightTemplates::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = RarityWeightTemplates::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = RarityWeightTemplates::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteRarityWeightTemplates(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new RarityWeightTemplates ; 
			$pky = $model->primaryKey ;
			$query = RarityWeightTemplates::where($pky , $id)->delete();
