...
class SkillLevelController extends Controller
{
    ...
    public function getAllSkillLevel(Request $request)
    {
        try
        {
    ...
    public function getOneSkillLevel(Request $request,$id)
    {
		try {
    ...
    public function postSkillLevel(Request $request){
	 ...
	 * @return Response
	 */
	public function putSkillLevel(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = SkillLevel::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = SkillLevel::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = SkillLevel::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteSkillLevel(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new SkillLevel ; 
			$pky = $model->primaryKey ;
			$query = SkillLevel::where($pky , $id)->delete();
