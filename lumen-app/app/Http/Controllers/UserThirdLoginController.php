...
class UserThirdLoginController extends Controller
{
    /**
    ...
    public function getAllUserThirdLogin(Request $request)
    {
        try
        {
    ...
    public function getOneUserThirdLogin(Request $request,$id)
    {
		try {
    ...
    public function postUserThirdLogin(Request $request){
	 ...
	 * @return Response
	 */
	public function putUserThirdLogin(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = UserThirdLogin::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = UserThirdLogin::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = UserThirdLogin::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteUserThirdLogin(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new UserThirdLogin ; 
			$pky = $model->primaryKey ;
			$query = UserThirdLogin::where($pky , $id)->delete();
