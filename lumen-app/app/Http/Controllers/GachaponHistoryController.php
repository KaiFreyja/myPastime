...
class GachaponHistoryController extends Controller
{
    /**
    ...
    public function getAllGachaponHistory(Request $request)
    {
        try
        {
    ...
    public function getOneGachaponHistory(Request $request,$id)
    {
		try {
    ...
    public function postGachaponHistory(Request $request){
	 ...
	 * @return Response
	 */
	public function putGachaponHistory(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = GachaponHistory::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = GachaponHistory::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = GachaponHistory::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteGachaponHistory(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new GachaponHistory ; 
			$pky = $model->primaryKey ;
			$query = GachaponHistory::where($pky , $id)->delete();
