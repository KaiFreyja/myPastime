...
use App\Models\SkillLevelBuff;

class SkillLevelBuffController extends Controller
{
    /**
    ...
    public function getAllSkillLevelBuff(Request $request)
    {
        try
        {
    ...
    public function getOneSkillLevelBuff(Request $request,$id)
    {
		try {
    ...
    public function postSkillLevelBuff(Request $request){
	 ...
	 * @return Response
	 */
	public function putSkillLevelBuff(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = SkillLevelBuff::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = SkillLevelBuff::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = SkillLevelBuff::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteSkillLevelBuff(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new SkillLevelBuff ; 
			$pky = $model->primaryKey ;
			$query = SkillLevelBuff::where($pky , $id)->delete();
