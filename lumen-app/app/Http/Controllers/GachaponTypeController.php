...
use App\Models\GachaponType;

class GachaponTypeController extends Controller
{
    /**
    ...
    public function getAllGachaponType(Request $request)
    {
        try
        {
    ...
    public function getOneGachaponType(Request $request,$id)
    {
		try {
    ...
    public function postGachaponType(Request $request){
	 ...
	 * @return Response
	 */
	public function putGachaponType(Request $request,$id){
		$api_name = $this->getApiName($request->path());
$this->validate($request, [
    'code' => 'integer',
    'name' => 'max:32',
    'seq' => 'integer',
		...
		try {
			...
			$check_id = GachaponType::where($pky , $id)->get();
			if(!$check_id || count($check_id) == 0){
				return response()->json(['message' => '此ID不存在。'], 422);
			}
			if($request->code){
				$check = GachaponType::where($pky,$id)->first();
				if($check->code != $request->code){
				 $check_code = GachaponType::where('code',$request->code)->get();
	 ...
	 * @return Response
	 */
	public function deleteGachaponType(Request $request,$id){
		$api_name = $this->getApiName($request->input("uri"));
		...
		try {
			...
			$model = new GachaponType ; 
			$pky = $model->primaryKey ;
			$query = GachaponType::where($pky , $id)->delete();
